...
public class TiledMap {
	...
	public String getTileProperty(int tileID, String propertyName, String def) {
		...
		Properties titleProps = set.getProperties(tileID);
		if (titleProps == null) {
			return def;
		}
		return titleProps.getProperty(propertyName, def);
	}
		...
		public TileSet(Element element) throws SlickException {
			...
			for (int i=0;i<pElements.getLength();i++) {
				...
				for (int p=0;p<properties.getLength();p++) {
					...
					String titleName = propElement.getAttribute("name");
					String value = propElement.getAttribute("value");

					tileProps.setProperty(titleName, value);
				}
			...
			if (propsElement != null) {
				...
				if (properties != null) {
					...
					for (int p = 0; p < properties.getLength();p++) {
						...
						String titleName = propElement.getAttribute("name");
						String value = propElement.getAttribute("value");		
						props.setProperty(titleName, value);
					}
				}
			}
